...
class StableDiffusionPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            # if is_debug and embedder is None:
            #     prompt = "In a mountainous landscape at dawn, soft sun rays filter through light mist, casting a warm glow on lush green slopes. Distant mountain ranges, layered in a soft blue hue, evoke depth and tranquility. A clear stream meanders gently through the valley, mirroring dense greenery."
            ...
            import pdb
            is_debug = False
            if is_debug and embedder is not None:
                mt5_attention_mask = torch.tril(attention_mask.expand(256, 256)).unsqueeze(0)

                prompt_embeds = text_encoder(
                    text_input_ids.to(device),
                    attention_mask=mt5_attention_mask,
                )
            else:
                # pdb.set_trace()
                prompt_embeds = text_encoder(
            ...
            # pdb.set_trace()
            # if is_debug and embedder is not None:
            #     with h5py.File("/data/xiaot/Methods/VLLMs/UMUG/data/test_data/predict/embedding_1226.1ar.h5", 'r') as f:
            #         numpy_array = f['image_embedding'][:]
            #     mt5_prompt_embeds = torch.tensor(numpy_array).to(torch.float16)

            #     prompt_embeds[:, :mt5_prompt_embeds.size(1), :] = mt5_prompt_embeds
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            if is_debug and embedder is not None:
                mt5_attention_mask = torch.tril(uncond_attention_mask.expand(256, 256)).unsqueeze(0)
    ...
    def __call__(
        ...
        if from_embedd is not None:
            # text_states = torch.zeros((1,333,1024)).to(self.device)
            # temp = from_embedd.to(text_states.dtype, self.device)


            if from_embedd.shape[0]==1:
                from_embedd = from_embedd.to(torch.float16)
                
                prompt_embeds_t5 = from_embedd

            # elif from_embedd.shape[0]==2:
            #     text_states_t5 = from_embedd.to(self.device, prompt_embeds_t5.dtype)
            else:
                return
            ...
            for i, t in enumerate(timesteps):
                ...
                if self.infer_mode in ["fa", "torch"]:
                    noise_pred = self.unet(
                        ...
                        return_dict=False,
                        is_get_embedding=is_get_embedding,
    ...
    def get_embedding(
        ...
        text_states_t5 = self.unet.mlp_t5(text_states_t5.view(-1, c_t5))
        ...
        text_embedding = torch.where(clip_t5_mask.unsqueeze(2), text_states, self.unet.text_embedding_padding.to(text_states))
    ...
    def embedding_to_image(
        ...
        num_images_per_prompt: Optional[int] = 1,
        prompt_embeds: Optional[torch.FloatTensor] = None,
        from_embedd: Optional[torch.FloatTensor] = None,
        prompt_embeds_t5: Optional[torch.FloatTensor] = None,
        negative_prompt_embeds: Optional[torch.FloatTensor] = None,
        negative_prompt_embeds_t5: Optional[torch.FloatTensor] = None,
        generator: Optional[Union[torch.Generator, List[torch.Generator]]] = None,
            ...
            for i, t in enumerate(timesteps):
                ...
                if self.infer_mode in ["fa", "torch"]:
                    ...
                    noise_pred = self.unet(
                        ...
                        encoder_hidden_states_t5=prompt_embeds_t5.contiguous(),
                        text_embedding_mask_t5=attention_mask_t5.contiguous(),
                        from_embedd= from_embedd.contiguous(),
                    )
                    from_embedd=None
                else:
                    raise ValueError("Unknown infer_mode: {self.infer_mode}")
...
class StableDiffusionPipelineWithoutEncoder(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin, FromSingleFileMixin):
    r"""
    Pipeline for text-to-image generation using Stable Diffusion.
    ...
    def __init__(
        ...
        super().__init__()
        # self.text_encoder, self.tokenizer, self.embedder_t5
        # ========================================================
        self.infer_mode = infer_mode
