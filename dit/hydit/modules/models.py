...
class HunYuanDiT(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    def forward(self,
        ...
        #         text_states_t5[1]=from_embedd[0]
        #     elif from_embedd.shape[0]==2:
        #         text_states_t5 = from_embedd.to(self.device, text_states.dtype)
        #     else:
        #         return
        ...
        #     # temp = from_embedd.to(text_states.dtype, self.device)
        #     if from_embedd.shape[0]==1:
        #         text_states[1]=from_embedd[0]
        #     elif from_embedd.shape[0]==2:
        #         text_states = from_embedd.to(self.device, text_states.dtype)
        #     else:
        #         return
        
        # text_states = text_states.to(torch.bfloat16)
        # text_states = text_states.to(torch.float16)


        # text_states.shape: torch.Size([2, 333, 1024])
...
class HunYuanDiTWithoutEncoder(ModelMixin, ConfigMixin, PeftAdapterMixin):
    """
    HunYuanDiT: Diffusion model with a Transformer backbone.
...
# class HunYuanDiT_Embedding(ModelMixin, ConfigMixin, PeftAdapterMixin):
#     """
#     HunYuanDiT: Diffusion model with a Transformer backbone.
#
#     Inherit ModelMixin and ConfigMixin to be compatible with the sampler StableDiffusionPipeline of diffusers.
...
#         text_states = torch.where(clip_t5_mask.unsqueeze(2), text_states, self.text_embedding_padding.to(text_states))
#
#         # text_states = text_states[:, :77, :]            # 第一种
#         # text_states = text_states[:, 77:, :]            # 第二种
#
#         return text_states
#
#     def initialize_weights(self):
