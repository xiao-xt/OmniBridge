...
def load_file(path: Union[str, bytes, _T], ROOT_IMAGE_DIR: str) -> Union[BytesIO, _T]:
    res = path
...
def load_video_hf(videos: List[str], video_load_backend = 'pyav'):
...
def load_video_cogvlm2(video: Union[str, bytes], num_frames = 24) -> np.ndarray:
...
def load_video_llava(video: Union[str, bytes], num_frames = 16) -> np.ndarray:
    import av
    video_io = load_file(video)
...
def load_video_minicpmv_mplug_owl3(video: Union[str, bytes], max_num_frames = 16):
